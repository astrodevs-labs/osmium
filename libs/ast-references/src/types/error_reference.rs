/**
 * ErrorReference.rs
 * Definition of ErrorReference struct
...
pub struct ErrorParameterReference {
    pub name: String,
    pub ty: Type,
...
pub struct ErrorReference {
    pub name: String,
    pub location: Location,
    pub parameters : Vec<ErrorParameterReference>,
 ...
 *****************************************************************************/
 ...
 impl ErrorReference {
    pub fn new(name: String, location: Location, contract: Option<&Rc<RefCell<ContractReference>>>, file: Option<&Rc<RefCell<FileReference>>>) -> ErrorReference {
        ErrorReference {
            name: name,
            location: location,
            parameters: Vec::new(),
    ...
    pub fn add_value(&mut self, name: String, ty: Type, storage: Option<Storage>, location: Location) {
        self.parameters.push(ErrorParameterReference {
            name: name,
            ty: ty,
            storage: storage,
            location: location,
...
impl fmt::Display for ErrorReference {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(f, "Error {} at {}", self.name, self.location)
    }
}

impl fmt::Debug for ErrorReference {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(f, "Error {} at {}", self.name, self.location)
    }
}

impl PartialEq for ErrorReference {
    fn eq(&self, other: &ErrorReference) -> bool {
        self.name == other.name && self.location == other.location && self.parameters == other.parameters && self.file == other.file
    }
}

impl PartialEq for ErrorParameterReference {
    fn eq(&self, other: &ErrorParameterReference) -> bool {
...
mod tests {
    ...
    #[test]
    fn add_value() {
        let mut result = ErrorReference::new(
            String::from("Test"),
            Location::new(
                String::from("test.sol"),
                Bound::new(0, 0),
