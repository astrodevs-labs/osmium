...
impl DependencyInstaller {
    ...
    pub fn install_missing_dependencies(mut self, config: &mut Config) -> bool {
        let DependencyInstaller { quiet, .. } = self;
    ...
    pub fn install(self, config: &mut Config, dependencies: Vec<Dependency>) -> Result<()> {
        let DependencyInstaller { no_git, no_commit, quiet, .. } = self;
...
pub fn install_missing_dependencies(config: &mut ProjectConfig, quiet: bool) -> bool {
    DependencyInstaller { quiet, ..Default::default() }.install_missing_dependencies(config.inner.get_mut())
}
