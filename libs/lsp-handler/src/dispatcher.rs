...
impl Dispatcher {
    ...
    pub fn setup<F>(&mut self, creators: Vec<F>) where F: FnOnce(Connection) -> Box<dyn Handler> {
...
impl Dispatcher {
    ...
    pub async fn on_request(&mut self, method: &str, params: JsValue) -> std::result::Result<JsValue, JsValue> {
        log(method);
        ...
        match method {
            ...
            _ => {
                Err(JsValue::from_str("Method not dispatched"))
            }
    ...
    pub async fn on_notification(&mut self, method: &str, params: JsValue) -> std::result::Result<(), JsValue> {
        log(method);
...
impl Handler for Dispatcher {
    fn initialize(&self, params: InitializeParams) -> Result<InitializeResult> {
        let mut res = vec![];
        self.connection.log_message(MessageType::INFO, "Dispatcher initializing");
        ...
        let tmp_cap = res.iter().filter(|i| i.capabilities.position_encoding.is_some()).collect::<Vec<&InitializeResult>>();
        if !tmp_cap.is_empty() {
            result.capabilities.position_encoding = tmp_cap[0].capabilities.position_encoding.clone();
        }
        let tmp_cap = res.iter().filter(|i| i.capabilities.text_document_sync.is_some()).collect::<Vec<&InitializeResult>>();
        if !tmp_cap.is_empty() {
            result.capabilities.text_document_sync = tmp_cap[0].capabilities.text_document_sync.clone();
        }
        let tmp_cap = res.iter().filter(|i| i.capabilities.selection_range_provider.is_some()).collect::<Vec<&InitializeResult>>();
        if !tmp_cap.is_empty() {
            result.capabilities.selection_range_provider = tmp_cap[0].capabilities.selection_range_provider.clone();
        }
        let tmp_cap = res.iter().filter(|i| i.capabilities.hover_provider.is_some()).collect::<Vec<&InitializeResult>>();
        if !tmp_cap.is_empty() {
            result.capabilities.hover_provider = tmp_cap[0].capabilities.hover_provider.clone();
        }
        let tmp_cap = res.iter().filter(|i| i.capabilities.completion_provider.is_some()).collect::<Vec<&InitializeResult>>();
        if !tmp_cap.is_empty() {
            result.capabilities.completion_provider = tmp_cap[0].capabilities.completion_provider.clone();
        }
        let tmp_cap = res.iter().filter(|i| i.capabilities.signature_help_provider.is_some()).collect::<Vec<&InitializeResult>>();
        if !tmp_cap.is_empty() {
            result.capabilities.signature_help_provider = tmp_cap[0].capabilities.signature_help_provider.clone();
        }
        let tmp_cap = res.iter().filter(|i| i.capabilities.definition_provider.is_some()).collect::<Vec<&InitializeResult>>();
        if !tmp_cap.is_empty() {
            result.capabilities.definition_provider = tmp_cap[0].capabilities.definition_provider.clone();
        }
        let tmp_cap = res.iter().filter(|i| i.capabilities.type_definition_provider.is_some()).collect::<Vec<&InitializeResult>>();
        if !tmp_cap.is_empty() {
            result.capabilities.type_definition_provider = tmp_cap[0].capabilities.type_definition_provider.clone();
        }
        let tmp_cap = res.iter().filter(|i| i.capabilities.implementation_provider.is_some()).collect::<Vec<&InitializeResult>>();
        if !tmp_cap.is_empty() {
            result.capabilities.implementation_provider = tmp_cap[0].capabilities.implementation_provider.clone();
        }
        let tmp_cap = res.iter().filter(|i| i.capabilities.references_provider.is_some()).collect::<Vec<&InitializeResult>>();
        if !tmp_cap.is_empty() {
            result.capabilities.references_provider = tmp_cap[0].capabilities.references_provider.clone();
        }
        let tmp_cap = res.iter().filter(|i| i.capabilities.document_highlight_provider.is_some()).collect::<Vec<&InitializeResult>>();
        if !tmp_cap.is_empty() {
            result.capabilities.document_highlight_provider = tmp_cap[0].capabilities.document_highlight_provider.clone();
        }
        let tmp_cap = res.iter().filter(|i| i.capabilities.document_symbol_provider.is_some()).collect::<Vec<&InitializeResult>>();
        if !tmp_cap.is_empty() {
            result.capabilities.document_symbol_provider = tmp_cap[0].capabilities.document_symbol_provider.clone();
        }
        let tmp_cap = res.iter().filter(|i| i.capabilities.workspace_symbol_provider.is_some()).collect::<Vec<&InitializeResult>>();
        if !tmp_cap.is_empty() {
            result.capabilities.workspace_symbol_provider = tmp_cap[0].capabilities.workspace_symbol_provider.clone();
        }
        let tmp_cap = res.iter().filter(|i| i.capabilities.code_action_provider.is_some()).collect::<Vec<&InitializeResult>>();
        if !tmp_cap.is_empty() {
            result.capabilities.code_action_provider = tmp_cap[0].capabilities.code_action_provider.clone();
        }
        let tmp_cap = res.iter().filter(|i| i.capabilities.code_lens_provider.is_some()).collect::<Vec<&InitializeResult>>();
        if !tmp_cap.is_empty() {
            result.capabilities.code_lens_provider = tmp_cap[0].capabilities.code_lens_provider;
        }
        let tmp_cap = res.iter().filter(|i| i.capabilities.document_formatting_provider.is_some()).collect::<Vec<&InitializeResult>>();
        if !tmp_cap.is_empty() {
            result.capabilities.document_formatting_provider = tmp_cap[0].capabilities.document_formatting_provider.clone();
        }
        let tmp_cap = res.iter().filter(|i| i.capabilities.document_range_formatting_provider.is_some()).collect::<Vec<&InitializeResult>>();
        if !tmp_cap.is_empty() {
            result.capabilities.document_range_formatting_provider = tmp_cap[0].capabilities.document_range_formatting_provider.clone();
        }
        let tmp_cap = res.iter().filter(|i| i.capabilities.document_on_type_formatting_provider.is_some()).collect::<Vec<&InitializeResult>>();
        if !tmp_cap.is_empty() {
            result.capabilities.document_on_type_formatting_provider = tmp_cap[0].capabilities.document_on_type_formatting_provider.clone();
        }
        let tmp_cap = res.iter().filter(|i| i.capabilities.rename_provider.is_some()).collect::<Vec<&InitializeResult>>();
        if !tmp_cap.is_empty() {
            result.capabilities.rename_provider = tmp_cap[0].capabilities.rename_provider.clone();
        }
        let tmp_cap = res.iter().filter(|i| i.capabilities.document_link_provider.is_some()).collect::<Vec<&InitializeResult>>();
        if !tmp_cap.is_empty() {
            result.capabilities.document_link_provider = tmp_cap[0].capabilities.document_link_provider.clone();
        }
        let tmp_cap = res.iter().filter(|i| i.capabilities.color_provider.is_some()).collect::<Vec<&InitializeResult>>();
        if !tmp_cap.is_empty() {
            result.capabilities.color_provider = tmp_cap[0].capabilities.color_provider.clone();
        }
        let tmp_cap = res.iter().filter(|i| i.capabilities.folding_range_provider.is_some()).collect::<Vec<&InitializeResult>>();
        if !tmp_cap.is_empty() {
            result.capabilities.folding_range_provider = tmp_cap[0].capabilities.folding_range_provider.clone();
        }
        let tmp_cap = res.iter().filter(|i| i.capabilities.declaration_provider.is_some()).collect::<Vec<&InitializeResult>>();
        if !tmp_cap.is_empty() {
            result.capabilities.declaration_provider = tmp_cap[0].capabilities.declaration_provider.clone();
        }
        let tmp_cap = res.iter().filter(|i| i.capabilities.workspace.is_some()).collect::<Vec<&InitializeResult>>();
        if !tmp_cap.is_empty() {
            result.capabilities.workspace = tmp_cap[0].capabilities.workspace.clone();
        }
        let tmp_cap = res.iter().filter(|i| i.capabilities.call_hierarchy_provider.is_some()).collect::<Vec<&InitializeResult>>();
        if !tmp_cap.is_empty() {
            result.capabilities.call_hierarchy_provider = tmp_cap[0].capabilities.call_hierarchy_provider;
        }
        let tmp_cap = res.iter().filter(|i| i.capabilities.semantic_tokens_provider.is_some()).collect::<Vec<&InitializeResult>>();
        if !tmp_cap.is_empty() {
            result.capabilities.semantic_tokens_provider = tmp_cap[0].capabilities.semantic_tokens_provider.clone();
        }
        let tmp_cap = res.iter().filter(|i| i.capabilities.moniker_provider.is_some()).collect::<Vec<&InitializeResult>>();
        if !tmp_cap.is_empty() {
            result.capabilities.moniker_provider = tmp_cap[0].capabilities.moniker_provider.clone();
        }
        let tmp_cap = res.iter().filter(|i| i.capabilities.linked_editing_range_provider.is_some()).collect::<Vec<&InitializeResult>>();
        if !tmp_cap.is_empty() {
            result.capabilities.linked_editing_range_provider = tmp_cap[0].capabilities.linked_editing_range_provider.clone();
        }        
        let tmp_cap = res.iter().filter(|i| i.capabilities.inline_value_provider.is_some()).collect::<Vec<&InitializeResult>>();
        if !tmp_cap.is_empty() {
            result.capabilities.inline_value_provider = tmp_cap[0].capabilities.inline_value_provider.clone();
        }
        let tmp_cap = res.iter().filter(|i| i.capabilities.inlay_hint_provider.is_some()).collect::<Vec<&InitializeResult>>();
        if !tmp_cap.is_empty() {
            result.capabilities.inlay_hint_provider = tmp_cap[0].capabilities.inlay_hint_provider.clone();
        }
        let tmp_cap = res.iter().filter(|i| i.capabilities.experimental.is_some()).collect::<Vec<&InitializeResult>>();
        if !tmp_cap.is_empty() {
    ...
    fn initialized(&self, params: InitializedParams) {
        self.connection.log_message(MessageType::INFO, "Dispatcher initialized");
        for handler in &self.handlers {
    ...
    fn will_save_wait_until(&self, params: WillSaveTextDocumentParams) -> Result<Option<Vec<TextEdit>>> {
    ...
    fn goto_declaration(&self, params: GotoDeclarationParams) -> Result<Option<GotoDeclarationResponse>> {
        for handler in &self.handlers {
    ...
    fn goto_type_definition(&self, params: GotoTypeDefinitionParams) -> Result<Option<GotoTypeDefinitionResponse>> {
        for handler in &self.handlers {
    ...
    fn goto_implementation(&self, params: GotoImplementationParams) -> Result<Option<GotoImplementationResponse>> {
        for handler in &self.handlers {
    ...
    fn prepare_call_hierarchy(&self, params: CallHierarchyPrepareParams) -> Result<Option<Vec<CallHierarchyItem>>> {
    ...
    fn incoming_calls(&self, params: CallHierarchyIncomingCallsParams) -> Result<Option<Vec<CallHierarchyIncomingCall>>> {
    ...
    fn outgoing_calls(&self, params: CallHierarchyOutgoingCallsParams) -> Result<Option<Vec<CallHierarchyOutgoingCall>>> {
    ...
    fn prepare_type_hierarchy(&self, params: TypeHierarchyPrepareParams) -> Result<Option<Vec<TypeHierarchyItem>>> {
    ...
    fn supertypes(&self, params: TypeHierarchySupertypesParams) -> Result<Option<Vec<TypeHierarchyItem>>> {
        let mut text_edit: Vec<TypeHierarchyItem> = vec![];
        for handler in &self.handlers {
    ...
    fn subtypes(&self, params: TypeHierarchySubtypesParams) -> Result<Option<Vec<TypeHierarchyItem>>> {
        let mut text_edit: Vec<TypeHierarchyItem> = vec![];
        for handler in &self.handlers {
    ...
    fn document_highlight(&self, params: DocumentHighlightParams) -> Result<Option<Vec<DocumentHighlight>>> {
    ...
    fn document_symbol(&self, params: DocumentSymbolParams) -> Result<Option<DocumentSymbolResponse>> {
        for handler in &self.handlers {
    ...
    fn semantic_tokens_full_delta(&self, params: SemanticTokensDeltaParams) -> Result<Option<SemanticTokensFullDeltaResult>> {
        for handler in &self.handlers {
    ...
    fn semantic_tokens_range(&self, params: SemanticTokensRangeParams) -> Result<Option<SemanticTokensRangeResult>> {
        for handler in &self.handlers {
    ...
    fn diagnostic(&self, params: DocumentDiagnosticParams) -> Result<DocumentDiagnosticReportResult> {
        for handler in &self.handlers {
    ...
    fn workspace_diagnostic(&self, params: WorkspaceDiagnosticParams) -> Result<WorkspaceDiagnosticReportResult> {
        for handler in &self.handlers {
    ...
    fn color_presentation(&self, params: ColorPresentationParams) -> Result<Vec<ColorPresentation>> {
    ...
    fn range_formatting(&self, params: DocumentRangeFormattingParams) -> Result<Option<Vec<TextEdit>>> {
        let mut text_edit: Vec<TextEdit> = vec![];
        for handler in &self.handlers {
    ...
    fn on_type_formatting(&self, params: DocumentOnTypeFormattingParams) -> Result<Option<Vec<TextEdit>>> {
        let mut text_edit: Vec<TextEdit> = vec![];
        for handler in &self.handlers {
    ...
    fn prepare_rename(&self, params: TextDocumentPositionParams) -> Result<Option<PrepareRenameResponse>> {
        for handler in &self.handlers {
    ...
    fn linked_editing_range(&self, params: LinkedEditingRangeParams) -> Result<Option<LinkedEditingRanges>> {
        for handler in &self.handlers {
