...
impl<S: LanguageServer> LspService<S> {
    ...
    pub  fn call_request(&self, method: &str, params: serde_json::Value) -> Result<Option<serde_json::Value>, Box<dyn Error>> {
        let ret = match method {
            ...
            "textDocument/incomingCalls" => {
                let params: CallHierarchyIncomingCallsParams = serde_json::from_value(params).unwrap();
                serde_json::to_value(self.inner.backend.incoming_calls(params)?)
            },
            "textDocument/outgoingCalls" => {
                let params: CallHierarchyOutgoingCallsParams = serde_json::from_value(params).unwrap();
            ...
            "textDocument/onTypeFormatting" => {
                let params: DocumentOnTypeFormattingParams = serde_json::from_value(params).unwrap();
    ...
    pub fn call_notification(&self, method: &str, params: serde_json::Value) -> Result<(), Box<dyn Error>> {
        match method {
            "initialized" => {
            ...
            "workspace/didChangeWorkspaceFolders" => {
                let params: DidChangeWorkspaceFoldersParams = serde_json::from_value(params).unwrap();
